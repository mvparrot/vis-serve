...
# sign of three numbers: sign(sign(cor.x) + sign(cor.y) + sign(cor.z) - 2) * abs(cor.x*cor.y*cor.z)

usefulness <- resultstidy %>% select(serveid,arc,usefulness) %>% distinct(serveid,arc,.keep_all=TRUE)
hist(usefulness$usefulness[usefulness$usefulness>0.8])
hist(usefulness$usefulness)

# usefulness_v <- usefulness
# resultstidy_v <- resultstidy
# save(usefulness_v,file=("Data usefulness_adj=(v).RData"))
# # save(resultstidy_v,file=("Data resultstidy_adj=(v).RData"))
# usefulness_meanv <- usefulness
# resultstidy_meanv <- resultstidy
# save(usefulness_meanv,file=("Data usefulness_adj=mean(v).RData"))
# save(resultstidy_meanv,file=("Data resultstidy_adj=mean(v).RData"))

hist(usefulness_v$usefulness[usefulness_v$usefulness>0.8], main = "Usefulness_v > 0.8", breaks = 100)
hist(usefulness_v$usefulness, main = "Usefulness_v", breaks = 1000)
hist(usefulness_meanv$usefulness[usefulness_meanv$usefulness>0.8], main = "Usefulness_meanv > 0.8", breaks = 100)
hist(usefulness_meanv$usefulness, main = "Usefulness_meanv", breaks = 1000)

usefulness_compare <-  full_join(usefulness_v,usefulness_meanv,by=c("serveid","arc"),suffix=c("_v","_meanv"))
save(usefulness_compare,file=("Data usefulness_compare.RData"))

require(ggplot2)
usefulness_compare_plot <- usefulness_compare %>% gather(model,usefulness,-serveid,-arc)
ggplot(usefulness_compare_plot, aes(usefulness, fill = factor(arc))) + 
    geom_density(alpha = 0.2) + facet_wrap(~model)
usefulness_compare_plot <- usefulness_compare %>% gather(model,usefulness,-serveid,-arc) %>% filter(usefulness > 0.8)
ggplot(usefulness_compare_plot, aes(usefulness, fill = factor(arc))) + 
    geom_density(alpha = 0.2) + facet_wrap(~model)
