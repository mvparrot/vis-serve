...
ax_p <- function(v,vx,vy,vz,w,wx,wy) {
    -D(v,w)/v * vx + L(v,w)*cnorm(wx,wy,wz(w,wx,wy),vx,vy,vz)*(wy*vz - wz(w,wx,wy)*vy)
}
ay_p <- function(v,vx,vy,vz,w,wx,wy) {
    -D(v,w)/v * vy + L(v,w)*cnorm(wx,wy,wz(w,wx,wy),vx,vy,vz)*(-wx*vz + wz(w,wx,wy)*vx)
}
az_p <- function(v,vx,vy,vz,w,wx,wy) {
    -D(v,w)/v * vz + L(v,w)*cnorm(wx,wy,wz(w,wx,wy),vx,vy,vz)*(wx*vy - wy*vx) - g
}
...
SSR <- function(data, par) {
    with(data, sum(
        (ax_p(v,vx,vy,vz,par[1],par[2],par[3]) - m*ax)^2 + 
        (ay_p(v,vx,vy,vz,par[1],par[2],par[3]) - m*ay)^2 +
        (az_p(v,vx,vy,vz,par[1],par[2],par[3]) - m*az)^2
    ))
}
